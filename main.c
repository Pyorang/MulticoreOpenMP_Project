...
int main(int argc, char** argv) {
    ...
    int width, height;
    uint8_t* img1 = load_pgm(argv[1], &width, &height);
    uint8_t* img2 = load_pgm(argv[2], &width, &height);
    ...
    save_pgm("diff.pgm", diff, width, height);
    save_pgm("binary.pgm", binary, width, height);
